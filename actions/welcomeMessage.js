const welcomeMessage = async (ctx, next) => {
  const { new_chat_members } = ctx.message;
  const newMembers = new_chat_members.filter( member => member.is_bot === false)
  var membersName = newMembers.map( usr => [usr.first_name,usr.last_name].join(" ") );

  const { message_id } = await ctx.replyWithMarkdown(`നമസ്കാരം ${membersName.join(', ')} LCHF മലയാളം ഗ്രൂപ്പിലേക്ക് സ്വാഗതം 🙏
